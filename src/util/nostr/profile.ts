...
import { getCachedNostrIdentity, saveNostrIdentity } from './store'
import { type NostrIdentity, truncateNpub } from './types'
...
export const refreshOwnNostrProfile = async (
  ...
  if (remote != null) {
    ...
    if (
      ...
      await saveNostrIdentity(account, next)
    }
    return remote
  }
...
export const publishNostrDisplayName = async (
  ...
  await saveNostrIdentity(account, {
    ...identity,
    displayName: trimmed,
    name: content.name,
...
export const getLocalIdentityProfile = (): NostrProfile | null => {
  const identity = getCachedNostrIdentity()
